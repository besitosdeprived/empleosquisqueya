import { bigint, pgEnum, pgTable, text, uuid, varchar } from "drizzle-orm/pg-core";
...
export const jobTable = pgTable("job", {
  ...
  authorId: bigint("author_id", { mode: "number" }).references(() => userTable.id).notNull(),
  createdAt,
  updatedAt
})


export const jobPostStatusEnum = pgEnum("job_post_status", ["published", "archived", "pending"])


export const jobPostInfoTable = pgTable("job_post_info", {
  id: bigIntPrimarykey,
  slug: uuid().defaultRandom().notNull(),
  jobId: bigint({ mode: "number" }).references(() => jobTable.id).notNull(),
