...
export const load = (async ({ url }) => {
	const jobTypeQueryParam = url.searchParams.getAll("job_type") as
		| SelectJob["jobType"][]
		| null;
	const pageQueryParam = url.searchParams.get("page");
	const orderByQueryParam = url.searchParams.get("order_by") as
		| "asc"
		| "desc"
		| null;
	const job: JobResultType[] = await db.query.jobTable.findMany({
		where: (t, { inArray }) =>
			jobTypeQueryParam?.length
				? inArray(t.jobType, [...jobTypeQueryParam])
				: undefined,
		columns: {
			title: true,
			salary: true,
